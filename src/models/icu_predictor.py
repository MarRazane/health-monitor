    ...
    for _ in range(n_patients):
        ...
        if is_deteriorating:
            ...
            if np.random.rand() > 0.3:
                patient["heart_rate"][onset:] += noisy_ramp(np.random.uniform(10, 35))
            if np.random.rand() > 0.3:
                patient["sbp"][onset:] -= noisy_ramp(np.random.uniform(10, 30))
                patient["dbp"][onset:] -= noisy_ramp(np.random.uniform(5, 15))
            if np.random.rand() > 0.4:
                patient["spo2"][onset:] -= noisy_ramp(np.random.uniform(3, 10))
            if np.random.rand() > 0.3:
                patient["resp_rate"][onset:] += noisy_ramp(np.random.uniform(4, 12))
            if np.random.rand() > 0.5:
                patient["temperature"][onset:] += noisy_ramp(np.random.uniform(0.3, 1.5))
        ...
        X.append(np.stack([patient[v] for v in VITALS], axis=1))
...
def train_model(X_train, y_train, X_val, y_val, epochs=60, batch_size=64, lr=1e-3):

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Training on: {device}")
    print(f"  Train : {len(X_train)}   Val : {len(X_val)}")
    print(f"  Deterioration rate — train : {y_train.mean():.2%}   val : {y_val.mean():.2%}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_auc > best_val_auc:
            ...
            torch.save(model.state_dict(), MODELS_PATH / "icu_predictor_best.pt")
    ...
    model.load_state_dict(
        torch.load(MODELS_PATH / "icu_predictor_best.pt", map_location=device)
    )
...
def evaluate(model, X_test, y_test, threshold=0.5):
    ...
    print(classification_report(y_test, pred, target_names=["Stable", "Deteriorating"], zero_division=0))

    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle(f"ICU Deterioration Predictor — ROC-AUC={roc:.4f}", fontweight="bold")

    RocCurveDisplay.from_predictions(
        y_test, probs, name=f"BiLSTM+Attention (AUC={roc:.3f})", ax=axes[0], color="steelblue"
    )
    axes[0].plot([0, 1], [0, 1], "k--", alpha=0.3)
    axes[0].set_title("ROC Curve")

    axes[1].hist(probs[y_test == 0], bins=40, color="green", alpha=0.6, label="Stable",        density=True)
    axes[1].hist(probs[y_test == 1], bins=40, color="red",   alpha=0.6, label="Deteriorating", density=True)
...
def plot_attention(model, X_test, y_test, n_cases=4):
    ...
    fig.suptitle("Attention Weights — Deteriorating Patients", fontweight="bold")
    ...
    for i in range(n_cases):
        ...
        # left column: normalised vital signs overlaid
        ax_v = axes[i, 0]
        ...
        # right column: attention weights as a bar chart
        ax_a = axes[i, 1]
        ax_a.bar(hours, attn, color="steelblue", alpha=0.7)
        ax_a.set_title("Attention weights (taller = more influential)")
...
if __name__ == "__main__":
    ...
    # save scaler parameters so we can apply the same transform at inference time
    ...
    model, history = train_model(X_train, y_train, X_val, y_val, epochs=60, batch_size=64)
