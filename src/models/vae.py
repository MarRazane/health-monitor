...
def elbo_loss(recon, target, mu, log_var, beta=1.0):
    ...
    recon_loss  = mse + 0.5 * F.mse_loss(grad_recon, grad_target, reduction="mean")
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for (batch,) in val_loader:
                batch = batch.to(device)
                recon, mu, log_var = model(batch)
                loss, _, _ = elbo_loss(recon, batch, mu, log_var, beta)
                v_loss += loss.item()
    ...
    model.load_state_dict(torch.load(MODELS_PATH / "vae_best.pt", map_location=device))
...
class VAEAnomalyScorer:
    ...
    def _score_batch(self, X, batch_size=256):
        ...
        return (
            np.concatenate(all_recon_mean),
            np.concatenate(all_recon_std),
            np.concatenate(all_kl),
        )

    def _combine(self, recon_mean, recon_std, kl_mean):
        raw    = np.stack([recon_mean, recon_std, kl_mean], axis=1)
        scaled = self.scaler.transform(raw)
        return 0.5 * scaled[:, 0] + 0.3 * scaled[:, 2] + 0.2 * scaled[:, 1]

    def fit(self, X_train):
        print("  Fitting VAE scorer (Monte Carlo sampling)...")
        rm, rs, km = self._score_batch(X_train)
        raw = np.stack([rm, rs, km], axis=1)
        self.scaler.fit(raw)
        self._train_scores = self._combine(rm, rs, km)
        print(f"  Done — mean={self._train_scores.mean():.4f}  std={self._train_scores.std():.4f}")

    def score(self, X):
        rm, rs, km = self._score_batch(X)
        return self._combine(rm, rs, km)
...
def evaluate(scorer, X_test, y_test, title="Conv1D VAE"):
    ...
    fig.suptitle(f"{title} — ROC-AUC={roc:.4f}  F1={f1:.4f}", fontweight="bold")

    axes[0].hist(scores[y_test == 0], bins=50, color="green", alpha=0.6, label="Normal",  density=True)
    axes[0].hist(scores[y_test == 1], bins=50, color="red",   alpha=0.6, label="Anomaly", density=True)
    axes[0].axvline(scorer.threshold, color="black", linestyle="--", label="Threshold")
    axes[0].set_title("Score Distributions")
    axes[0].set_xlabel("ELBO Anomaly Score")
    axes[0].legend()

    RocCurveDisplay.from_predictions(
        y_test, scores, name=f"VAE (AUC={roc:.3f})", ax=axes[1], color="purple"
    )
    axes[1].plot([0, 1], [0, 1], "k--", alpha=0.3)
    axes[1].set_title("ROC Curve")

    axes[2].plot(sorted(scorer._train_scores), color="steelblue", alpha=0.7, linewidth=0.8)
    axes[2].axhline(scorer.threshold, color="red", linestyle="--", label="Threshold")
    ...
    plt.show()
    print("Saved → models/vae_evaluation.png")
    return roc, f1
...
def plot_latent_space(model, X_test, y_test, device="cpu"):
    ...
    print("  Running t-SNE on latent codes...")
    model.eval()
    ...
    Z_2d   = TSNE(n_components=2, random_state=42, perplexity=30).fit_transform(Z[idx])
    labels = y_test[idx]

    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle("VAE Latent Space (t-SNE)", fontweight="bold")

    for cls, name, color in [(0, "Normal", "steelblue"), (1, "Anomaly", "red")]:
        mask = labels == cls
        axes[0].scatter(Z_2d[mask, 0], Z_2d[mask, 1], c=color, label=name, alpha=0.4, s=8)
    axes[0].set_title("Normal vs Anomaly in Latent Space")
    axes[0].legend()

    norms = np.linalg.norm(Z[idx], axis=1)
    axes[1].hist(norms[labels == 0], bins=40, color="steelblue", alpha=0.6, label="Normal",  density=True)
    axes[1].hist(norms[labels == 1], bins=40, color="red",       alpha=0.6, label="Anomaly", density=True)
    ...
    plt.show()
    print("Saved → models/vae_latent_space.png")
...
def compare_all_models(scorers_dict, X_test, y_test):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle("All Models Comparison", fontweight="bold")

    colors  = {"CNN AE": "steelblue", "Transformer": "darkorange", "VAE": "purple"}
    summary = {}
    ...
    for name, scorer in scorers_dict.items():
        ...
        RocCurveDisplay.from_predictions(
            y_test, scores, name=f"{name} (AUC={roc:.3f})", ax=axes[0], color=colors[name]
        )
    ...
    plt.show()
    print("Saved → models/all_models_comparison.png")
    return summary
...
if __name__ == "__main__":
    ...
    # train VAE
    print("\nTraining Conv1D VAE")
    vae, history = train_model(X_train, X_val, epochs=80, batch_size=128, max_samples=20000, beta=0.5)
    ...
    X_thr, y_thr   = X_test[:split],  y_test[:split]
    X_eval, y_eval = X_test[split:],  y_test[split:]
    vae_scorer.optimize_threshold(X_train[:10000])
    print("\nEvaluating VAE")
    ...
    if Path(trans_path).exists():
        ...
        trans_scorer.optimize_threshold(X_train[:10000])
        scorers_dict["Transformer"] = trans_scorer
        print("  Transformer loaded")
    else:
        print(" skipping")

    if len(scorers_dict) > 1:
        compare_all_models(scorers_dict, X_test, y_test)