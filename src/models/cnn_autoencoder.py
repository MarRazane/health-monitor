...
class CNNAutoencoder(nn.Module):
    ...
    def __init__(self, seq_len=256, latent_dim=8):
        ...
        self.bottleneck_enc = nn.Linear(128 * self._latent_spatial, latent_dim)
        self.bottleneck_dec = nn.Linear(latent_dim, 128 * self._latent_spatial)
...
def train_model(X_train, X_val, epochs=100, batch_size=128, lr=1e-3, max_samples=50000):
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for (batch,) in val_loader:
                batch = batch.to(device)
                recon = model(batch)
                running_val += reconstruction_loss(recon, batch).item()
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"Epoch {epoch:>3}/{epochs}  train={avg_train:.5f}  val={avg_val:.5f}")

    model.load_state_dict(torch.load(MODELS_PATH / "cnn_ae_best.pt", map_location=device))
...
class AnomalyScorer:
    ...
    def _forward(self, X):
        ...
        X_t = torch.tensor(X[:, np.newaxis, :], dtype=torch.float32).to(self.device)

        with torch.no_grad():
            z     = self.model.encode(X_t)
            recon = self.model.decode(z)

        return X_t.cpu().numpy(), recon.cpu().numpy(), z.cpu().numpy()

    def _raw_scores(self, Xo, Xr, Z):
        mse     = np.mean((Xo - Xr) ** 2, axis=(1, 2))
        max_err = np.max(np.abs(Xo - Xr), axis=(1, 2))
        grad    = np.mean((np.diff(Xo, axis=2) - np.diff(Xr, axis=2)) ** 2, axis=(1, 2))
    ...
    def score(self, X):
        ...
        raw = self._raw_scores(Xo, Xr, Z)
        return self._combine(self.scaler.transform(raw))
...
if __name__ == "__main__":
    ...
    model, history, val_history = train_model(X_train, X_val, epochs=100)
